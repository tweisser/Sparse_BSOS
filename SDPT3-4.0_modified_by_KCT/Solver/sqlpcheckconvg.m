...
%%
      ...
      if (stoplevel) & (iter > 2) & (~breakyes)
         ...
         if (iter > 20*2) & (infeas < 1e-4 | prim_infeas_bad) ...
            & (max(infeas,relgap) < 1) ...
	     ...
	     if (iter > 20*2) & (gap_progress_bad2) ...
 	        & (numpertdiagschur > 10);
            msg = 'stop: progress is bad*'; 
            if (printlevel); fprintf('\n  %s',msg); end
            termcode = -5;
	     ...
	     if (iter > 50) & (prim_infeas_bad) & (gap_slow) & (relgap < 1e-3) ...
	     ...
	     if (iter > 50) & (dual_infeas_bad) & (relgap < 1e-3) ...
	        & (dual_infeas < 1e-5) ...
            & ~(min(runhist.step(idx)) > 0.2 & ublksize)
